...
class DisplayOutputPanel::Impl {
public:
  using SinkSettingsMap = std::unordered_map<std::string, std::pair<int, bool>>;

  Impl(const dvrk_data::AppConfig &cfg, std::string settings_suffix,
       std::function<void()> quit_cb)
      : m_cfg(cfg), m_settings_suffix(std::move(settings_suffix)),
        m_quit_cb(std::move(quit_cb)),
        m_sink_controls_box(Gtk::ORIENTATION_VERTICAL) {
  ...
  void rebuild(const std::vector<DisplayOutputPanel::SinkDescriptor> &descriptors) {
    ...
    if (!m_sink_controls.empty()) {
      m_monitor_poll_connection = Glib::signal_timeout().connect(
          sigc::mem_fun(*this, &Impl::poll_window_monitors), 1000);
    }
...
private:
  struct SinkDisplayControl {
    ...
    std::unique_ptr<dvrk_data::WindowMonitorManager> monitor_manager;
  };
  ...
  void save_persisted_settings() {
    for (auto &control : m_sink_controls) {
      if (control.monitor_combo && control.monitor_manager) {
        ...
        control.monitor_manager->set_fullscreen(
  ...
  void append_sink_control_row(SinkDisplayControl &control,
    ...
    control.monitor_manager = std::make_unique<dvrk_data::WindowMonitorManager>(
    ...
    const auto previous = previous_settings.find(control.sink_name);
    if (previous != previous_settings.end()) {
      initial_monitor = previous->second.first;
      initial_fullscreen = previous->second.second;
    } else {
      control.monitor_manager->load_settings("sink:" + control.sink_name);
      initial_monitor = control.monitor_manager->get_monitor_index();
      initial_fullscreen = control.monitor_manager->get_fullscreen();
    }

    if (!m_monitor_labels.empty()) {
      initial_monitor = clip_int(
          initial_monitor, 0, static_cast<int>(m_monitor_labels.size()) - 1);
    } else {
      initial_monitor = 0;
    }
...
DisplayOutputPanel::DisplayOutputPanel(const dvrk_data::AppConfig &cfg,
                                       std::string settings_suffix,
                                       std::function<void()> quit_cb)
    : m_impl(std::make_unique<Impl>(cfg, std::move(settings_suffix),
                                    std::move(quit_cb))) {}

DisplayOutputPanel::~DisplayOutputPanel() = default;
DisplayOutputPanel::DisplayOutputPanel(DisplayOutputPanel &&) noexcept = default;
DisplayOutputPanel &DisplayOutputPanel::operator=(DisplayOutputPanel &&) noexcept = default;

Gtk::Frame &DisplayOutputPanel::widget() { return m_impl->widget(); }

void DisplayOutputPanel::rebuild(
    const std::vector<DisplayOutputPanel::SinkDescriptor> &descriptors) {
  m_impl->rebuild(descriptors);
}
